...
function dump($variable, $strlen = 100, $width = 25, $depth = 10, $i = 0, &$objects = array()) {
    ...
    case 'array':
      ...
      elseif (!$len) $string .= 'array(0) {}';
        ...
        foreach ($keys as $key) {
          ...
          $string .= dump($variable[$key], $strlen, $width, $depth, $i + 1, $objects);
          $count++;
        }
        $string .= '<br/>' . $spaces . '}';
      }
  ...
  if ($caller) $string = $caller['file'] . ':' . $caller['line'] . '<br/>' . $string;
